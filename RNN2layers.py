...
class RNN:
    def __init__(self, flavour='gru', N_HIDDEN=50, N_HIDDEN2=100, NUM_EPOCHS=500, patient=None):
        self.rnn = None
    ...
    def make_rnn(self, X, y, operation = T.add, peepholes=True):
        ...
        rnn['noise2'] = lasagne.layers.GaussianNoiseLayer(rnn['merge'], sigma=1)
	...
	if N_HIDDEN2 > 0:
		...
		rnn['merge2'] = lasagne.layers.ElemwiseMergeLayer([rnn['rnn_forward2'], rnn['rnn_backward2']], T.add)
		rnn['noise3'] = lasagne.layers.GaussianNoiseLayer(rnn['merge2'], sigma=1)
		rnn['dropout3'] = lasagne.layers.DropoutLayer(rnn['noise3'], p=0.5)
		rnn['reshape'] = ReshapeLayer(rnn['dropout3'], (-1, N_HIDDEN2))

	else:
		rnn['dropout3'] = lasagne.layers.DropoutLayer(rnn['noise2'], p=0.5)
		rnn['reshape'] = ReshapeLayer(rnn['dropout3'], (-1, N_HIDDEN))
    ...
    def fit(self, X, y, valid_set_x, valid_set_y, key=0, EPOCH_SIZE=1, max_patience=25):
        ...
        #updates = lasagne.updates.rmsprop(cost, all_params, 0.0001)
        ...
        print("Epoch {} Best validation accuracy = {}".format(epoch + 1, acc_val))
